...
xy100 = [[i,j]/100 for i in 0:100, j in 0:100][:]
...
AnS = reshape(analytic_solution(xy),21,21)
...
z(x) = x[1]*(1-x[1])*x[2]*(1-x[2])
Z = z.(xy)
Z = reshape(Z,101,101)
plt = heatmap(Z, xscale=0.1, yscale=0.1, xoffset=0, colormap=:inferno);   display(plt);
