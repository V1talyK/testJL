list_of_cellslv_kernel = "__kernel void slvk ( __global float *zz,
...
low_slv_kernel = "__kernel void slv_lowM( __global float *zz,
                                   __global const uint *list_of_cell,
                                   __global const uint *ind_new_lvl,
                                   __global const uint *lvl_lng,
                                   __global float *x,
                                   __global const float *b,
                                   __global const uint *cl1,
                          ...
                          __local float *lcly)
  {
   ...
   for (uint j = 0; j<sdf[0]; j++)
       ...
       if (lvl_lng[j]>64)
         ...
         if (lvl_lng[j]<=64 && lvl_lng[j]>=2)
             {
              ...
              for (uint jj = 0; jj<ss+1; jj++)
                 ...
                 if ((lc_id-(prt*mgr_sz)+jj*mgr_sz) < (cl1[trow+1]-2 - (cl1[trow]-1)))
                     ...
                     //lcly[lc_id] = nz[i2];
                     lclx[lc_id] = x[rw[i2]+step];
                    // barrier(CLK_LOCAL_MEM_FENCE);
                     localSums[lc_id] += x[rw[i2]+step]*nz[i2];
                     }
                  }
              for (uint stride = mgr_sz/2; stride>0; stride >>= 1)
                 {
                  barrier(CLK_LOCAL_MEM_FENCE);
         ...
         for (uint stride = gr_sz/2; stride>0; stride >>= 1)
            {
             barrier(CLK_LOCAL_MEM_FENCE);
             if (lc_id < stride)
               localSums[lc_id] += localSums[lc_id + stride];
